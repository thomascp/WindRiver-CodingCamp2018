...
function clearThumbnails(jsonData) {
	$("#thumbnail_section").html("");
}
...
// Get Information from HDC , so update it
function updateHDCInfo(hdcRetJsonObj)
{    
    var i = (hdcRetJsonObj.data.currentItemCount - 1);
        
    do
        {
        jsonDataPerment.photos[updateRoundRobin-1].url = base64decode(hdcRetJson.data.items[i].value);
        jsonDataPerment.photos[updateRoundRobin-1].thumb_url = base64decode(hdcRetJson.data.items[i].value);
        jsonDataPerment.photos[updateRoundRobin-1].date = hdcRetJson.data.items[i].timestamp;

        displayCurrentPhoto(updateRoundRobin.toString(), jsonDataPerment); // Displays the selected photo
        clearThumbnails(jsonDataPerment);
        displayThumbnails(jsonDataPerment);
        highlightSelectedThumbnail(updateRoundRobin.toString(), jsonDataPerment.photos.length);
        
        updateRoundRobin++;
        
        if (7 == updateRoundRobin)
            {
            updateRoundRobin = 1;
            }
    ...
    while ((i >= 0) && (jsonDataPerment.photos[updateRoundRobin-1].url == "images/nosig.png"));
    
    $('#thumbnail_section ul li img').click(function(){
        var imgIdClickedOn = $(this).attr('id');
        highlightSelectedThumbnail(imgIdClickedOn, jsonDataPerment.photos.length);
        displayCurrentPhoto(imgIdClickedOn, jsonDataPerment); // Displays the selected photo
    });
};
